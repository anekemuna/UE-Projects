...
int main(int argc, char * argv[])
{
   ...
   int first = 0; // Extra credit: local variable- checks if it's user's first
                  //                               move
   ...
   do
   {
	 ...
	 first++; // Extracredit: each uncover, increments first
	 
	 try
	 {
	    // Extra credit: first move  of a player does not uncover a bomb
	    if(first == 1)
	    {
	       game_grid.no_bomb(row, col);
	    }

	    // Extra credit: recursively uncover
	    game_grid.auto_open_recursive(row, col);  // 7.1 Uncover cell
